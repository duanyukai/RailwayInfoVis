...
const width = 1000; const radius = width / 2;
...
export function radial_tree(data, div_id) {
  const tree = d3.tree()
    .size([2 * Math.PI, radius])
    .separation((a, b) => (a.parent === b.parent ? 1 : 2) / a.depth);

  // root = tree(d3.hierarchy(data).sort((a, b) => d3.ascending(a.data.name, b.data.name)));
  let root = d3.hierarchy(data);// .sort((a, b) => d3.ascending(a.data.name, b.data.name));
  ...
  function update(source) {
    const duration = d3.event && d3.event.altKey ? 2000 : 500;
    tree(root);
  ...
  d3.select(`#${div_id}`).select("svg").remove(); // dyk 移除旧的svg
